...
contract Bouncer {
    ...
    // Entry 1 ETH 10 ether
    // Entry 2 ETH 10 ether
    ...
    function convert(address who, uint256 id) public payable {
        ...
        tokens[who][address(entry.token)] += entry.amount; // 40 ETH
    }
    ...
    function proofOfOwnership(ERC20Like token, address from, uint256 amount) public payable {
        if (address(token) == ETH) {
            ...
            require(token.transferFrom(from, address(this), amount), "err/not enough tokens"); // This is also possible to reentrant
        }
    }
...
contract Party {
    ...
    function isAllowed(address who) public view returns (bool) {
        ...
        res[0] = 0x6B175474E89094C44Da98b954EedeAC495271d0F; // DAI
        res[1] = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; // WETH
